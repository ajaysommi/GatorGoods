...
function AccountVerification() {
  ...
  const [message, setMessage] = useState('Enter UFID and Password')
  ...
  return (
    <div className="verification-page">
      <h1>GatorGoods {isRegister ? 'Sign Up' : 'Login'}</h1>
      <form onSubmit={handleSubmit}>
        <input
          type="text"
          value={ufid}
          onChange={(e) => setUfid(e.target.value)}
          placeholder="UFID (8 digits)"
          maxLength="8"
          required
        />
        <input
      ...
      <button onClick={() => setIsRegister(!isRegister)} style={{ marginTop: '1rem' }}>
        {isRegister ? 'Already have an account? Log in' : 'New to the swamp? Create account'}
      </button>
    </div>
  )
}
